...
from ytsync.modules import exceptions
...
class CronExpression:
    ...
    def __init__(self, line: str, epoch: tuple = DEFAULT_EPOCH, epoch_utc_offset: int = 0):
        ...
        if len(fields) < 6:
            raise exceptions.InvalidArgument(f"{line!r} has invalid cron expression!")
    ...
    def compute_numtab(self):
        ...
        for field_str, span in zip(self.string_tab, self.FIELD_RANGES):
            ...
            if len(split_field_str) > 1 and "*" in split_field_str:
                raise exceptions.InvalidArgument('"*" must be alone in a field.')
...
def parse_atom(parse: str, minmax: tuple) -> set | None:
    ...
    if parse == "*":
        ...
        if minmax[0] <= value <= minmax[1]:
            ...
            raise exceptions.InvalidArgument(f"invalid bounds: {parse}")
        ...
        if "-" in subrange:
            ...
            raise exceptions.InvalidArgument(f"unrecognized symbol: {subrange}")
